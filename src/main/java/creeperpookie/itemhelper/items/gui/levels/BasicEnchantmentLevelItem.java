...
public class BasicEnchantmentLevelItem implements CustomItem
{
	...
	public ItemStack getItemStack(int data)
	{
		...
		item.editMeta(BlockDataMeta.class, meta ->
		{
			...
			meta.displayName(Component.text("Level", DefaultTextColor.AQUA).appendSpace().append(Component.text(Utility.getLevelFromLightLevel(data)).color(DefaultTextColor.DARK_AQUA)).decoration(TextDecoration.ITALIC, false).decorate(TextDecoration.BOLD));
			meta.lore(List.of());
		});
